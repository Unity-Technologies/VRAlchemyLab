...
namespace UnityEngine.XR.Interaction
{
    ...
    public sealed class SnapTurnProvider : LocomotionProvider
    {
        ...
        InputDeviceRole m_PrimaryDeviceRole;
        /// <summary>
        /// The device role that will be used to locate the "Primary" controller device to use for snap turning.
        /// </summary>
        public InputDeviceRole PrimaryDeviceRole { get { return m_PrimaryDeviceRole; } set { m_PrimaryDeviceRole = value; TryResolveDevice(m_PrimaryDeviceRole,out m_CurrentPrimaryDevice); } }
        ...
        InputDeviceRole m_SecondaryDeviceRole;
        /// <summary>
        /// The device role that will be used to locate the "Secondary" controller device to use for snap turning.
        /// </summary>
        public InputDeviceRole SecondaryDeviceRole { get { return m_SecondaryDeviceRole; } set { m_SecondaryDeviceRole = value; TryResolveDevice(m_SecondaryDeviceRole, out m_CurrentSecondaryDevice); } }
        ...
        {
            base.Awake();
            TryResolveDevice(m_PrimaryDeviceRole, out m_CurrentPrimaryDevice);
            TryResolveDevice(m_SecondaryDeviceRole, out m_CurrentSecondaryDevice);
        }
        ...
        private void TryResolveDevice(InputDeviceRole role, out InputDevice device)
        {
            List<InputDevice> inputDeviceList = new List<InputDevice>();
            InputDevices.GetDevicesWithRole(role, inputDeviceList);
            if (inputDeviceList != null && inputDeviceList.Count > 0)
            {
                device = inputDeviceList[0];
                return;
            }
