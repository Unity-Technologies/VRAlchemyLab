...
namespace UnityEngine.XR.Interaction
{
    ...
    public class XRGrabInteractable : XRBaseInteractable
	{
        ...
        {
            if (isSelected)
                MoveToTarget(Time.fixedUnscaledDeltaTime);
        }
        ...
        void MoveToTarget(float timeDelta)
        {
            switch (m_CurrentMovementType)
            {
                ...
                case XRBaseInteractable.MovementType.NonKinematic:
                    {
        ...
        void SmoothVelocityUpdate()
        {
            ...
            m_ThrowSmoothingAngularVelocityFrames[m_ThrowSmoothingCurrentFrame] = (m_SelectingInteractor.attachTransform.rotation.eulerAngles - m_LastRotation.eulerAngles) * Mathf.Deg2Rad / Time.deltaTime;
