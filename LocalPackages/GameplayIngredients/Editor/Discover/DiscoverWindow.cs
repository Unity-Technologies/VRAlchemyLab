...
namespace GameplayIngredients.Editor
{
	...
	{
		// Serialized so the reference survives domain reloads
		[SerializeField]
		...
		static List<DiscoverWindow> s_Windows = new List<DiscoverWindow>();

		public static void SelectDiscover(Discover discover)
		{
			if (discover == null || s_Windows == null)
				return;

			foreach (var window in s_Windows)
			{
				if (window == null || window.discoverObjects == null)
					continue;

				foreach (var categoryKvp in window.discoverObjects)
				{
					if (categoryKvp.Value != null && categoryKvp.Value.Contains(discover))
					{
		...
		static void InitShowAtStartup()
		{
			...
			foreach (var guid in guids)
			{
				...
				if (asset != null && asset.EnableShowAtStartup)
				{
		...
		public static void ShowDiscoverWindow(DiscoverAsset asset)
		{
			if (asset != null)
			{
				var window = GetWindow<DiscoverWindow>(!asset.dockable);
				window.SetDiscoverAsset(asset);
			}
			else
			{
		...
		// Assigns the DiscoverAsset and configures the window
		...
		{
			// Safety: discard if asset or object is missing
			if (asset == null || discoverObject == null)
				return true;
		...
		{
			// Prevent null reference when window is restored without an asset
			if (discoverAsset == null)
				return;
		...
		private void OnGUI()
		{
			// If no asset is assigned, show a safe message instead of crashing
			if (discoverAsset == null)
			{
				EditorGUILayout.HelpBox("No DiscoverAsset assigned to this window.\nOpen a DiscoverAsset to initialize this window.", MessageType.Info);
							...
							using (new GroupLabelScope(map.Title))
							{
								...
								using (new GUILayout.HorizontalScope())
								{
									...
									if (map.SceneSetups != null)
									{
										...
										{
											// EditorSceneSetup is a struct or class custom, but array elements can be default
		...
		void SetSelectedDiscover(Discover newSelection)
		{
			...
			if (selectedDiscover != null && selectedDiscover.ObjectsToToggle != null)
			{
